...
def detection_bounding_boxes(image_path, output_dict):
	...
	for index, scores in enumerate(scores):
		if scores > 0.5 and not category_index[classes[index]]['name'] in str(boxes_list):
			ymin, xmin, ymax, xmax = boxes[index]
			label = category_index[classes[index]]['name']
			boxes_conv = label, scores, int(xmin * image_width), int(ymin * image_height), int(xmax * image_width), int(ymax * image_height)
			boxes_list.append(boxes_conv)
		else:
			continue
	print(boxes_list)
...
if __name__ == '__main__':
	...
	parser.add_argument('--server_url', dest='server_url', type=str, default ='http://172.28.1.3:8082/v1/models/default',
	...
	parser.add_argument('--label_map', dest='label_map', type=str, default='/tensorflow/workspace/training/annotations/label_map.pbtxt',
                        help='Path to the label map, which is json-file that maps each category name '
                             'to a unique number.')
	parser.add_argument('--image_path', dest='image_path', type=str, default="./input_img_document.jpg",
	...
	detection_bounding_boxes(load_image, output_dict)
