...
def create_pipeline():
    ...
    reset(output_path, image_dir, csv_input, tf_serving_model_path)
def data(output_path, image_dir, csv_input):
    """
    ...
    train_writer.close()
    eval_writer.close()
    print('successfully created the TFRecord: ' + output_path + '/train.record' + ';' + output_path + '/eval.record')
def preprocessmodel(tf_serving_model_path, output_path):
    """
    ...
    print('found latest model:'+ tmpDir)
    mkdir(output_path + '/model/')
    copy_tree(tmpDir, output_path + '/model/')
    print('preprocess: '+str(copy_tree(tmpDir, output_path + '/model/')))
def train(pipeline_config_path, model_dir, num_train_steps):
    """
...
def monitor(model_dir):
    ...
    """
    print('start TensorBoard on port: 6006')
    tb = program.TensorBoard()
    tb.configure(argv=[None, '--logdir', model_dir, '--port', str(6006)])
    url = tb.launch()
    print(url)
def modelexporter(pipeline_config_path, model_dir, output_path):
    """
    ...
    try:
        ...
        print('export of new model failed!' +str(e))
def trainmover(model_dir, output_path):
    shutil.copytree(model_dir,os.path.join(output_path, 'push', 'training'))
def pusher(output_path,tf_serving_model_path):
    try:
        ...
        print('push of new model failed!' +str(e))
def clean(image_dir,csv_input):
    try:
        rmdir(image_dir)
        rmfile(csv_input)
    except Exception as e:
        print('clean of image data and annotation.csv failed!' +str(e))
"""
...
def newversion(tf_serving_model_path):
    try:
        ...
        print('find new model version failed!' + str(e))
def modelmover(output_path):
    """
    ...
    try:
        ...
        print('directory move of new model failed!' +str(e))
def reset(output_path, image_dir, csv_input, tf_serving_model_path):
    """
...
def shaffelfiles(path, summ):
    ...
    for x in range(0, summ):
        ...
        while not files[index].endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp')) or cnt == 100:
            cnt += 1
            index = random.randrange(0, len(files))
