...
def data(output_path, image_dir, csv_input, **context):
    ...
    try:
        ...
        logging.info('data processing failed!' +str(e))
        return False
...
task_reset = PythonOperator(task_id='reset',provide_context=True,python_callable=reset,op_kwargs={"output_path":output_path, "image_dir":image_dir, "csv_input":csv_input, "tf_serving_model_path":tf_serving_model_path},dag=dag)
task_data = PythonOperator(task_id='data',provide_context=True,python_callable=data,op_kwargs={"output_path":output_path, "image_dir":image_dir, "csv_input":csv_input},dag=dag)

task_reset >> task_data
#reset(output_path, image_dir, csv_input, tf_serving_model_path)
#data(output_path, image_dir, csv_input)
#preprocessmodel(tf_serving_model_path, output_path)
#monitor(model_dir)
